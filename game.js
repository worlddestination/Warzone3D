...
const STATE = {
  ...
  shootCooldown:0,
  grenadeTimer:0,
  stingerLocked:false, stingerTarget:null,
};

const WEAPONS = [
  { name:'M4A1 ASSAULT RIFLE', ammo:30,  reserve:120, damage:22, fireRate:0.10, reloadTime:2.2, range:80,  spread:0.012, auto:true,  color:0x2a3040 },
  { name:'AWM SNIPER RIFLE',   ammo:5,   reserve:25,  damage:95, fireRate:1.2,  reloadTime:3.5, range:200, spread:0.001, auto:false, color:0x334422 },
  { name:'M249 SAW LMG',       ammo:100, reserve:300, damage:18, fireRate:0.07, reloadTime:4.5, range:60,  spread:0.020, auto:true,  color:0x1a2030 },
  { name:'FIM-92 STINGER',     ammo:1,   reserve:4,   damage:0,  fireRate:1.5,  reloadTime:3.0, range:500, spread:0,     auto:false, color:0x2a3a20 },
];

const ENEMY_TYPES = [
  { name:'RIFLEMAN',  bodyColor:0x4a5a3a, uniformColor:0x3a4a2c, helmetColor:0x2a3a20, vestColor:0x384830, health:60,  speed:3.0, damage:8,  score:100, scale:1.0  },
  { name:'SERGEANT',  bodyColor:0x3a4a2c, uniformColor:0x2e3d22, helmetColor:0x1e2e12, vestColor:0x2a3820, health:100, speed:2.6, damage:14, score:200, scale:1.05 },
  { name:'HEAVY GUN', bodyColor:0x2a3820, uniformColor:0x1e2e12, helmetColor:0x141e0a, vestColor:0x1e2a12, health:180, speed:1.8, damage:20, score:350, scale:1.3  },
  { name:'SNIPER',    bodyColor:0x3a4020, uniformColor:0x2c3218, helmetColor:0x1a2010, vestColor:0x283018, health:80,  speed:2.0, damage:35, score:300, scale:0.98 },
  { name:'COMMANDER', bodyColor:0x1a2818, uniformColor:0x141e10, helmetColor:0x0e1608, vestColor:0x18200e, health:220, speed:2.8, damage:25, score:600, scale:1.2  },
  { name:'SUB GUN',   bodyColor:0x3c4c2c, uniformColor:0x303e20, helmetColor:0x202e14, vestColor:0x283418, health:55,  speed:3.5, damage:10, score:120, scale:0.95 },
];
...
function buildLighting() {
  ...
  const cfgs = [
    {x:-20,y:2,z:-20,c:0x40ff80,i:3.0,d:18},{x:25,y:1,z:15,c:0x80ff40,i:2.5,d:14},
    {x:-30,y:3,z:25,c:0x20ff60,i:2.8,d:16},{x:10,y:2,z:-35,c:0x60ff20,i:2.2,d:12},
    {x:-10,y:1,z:40,c:0x40cc80,i:2.0,d:15},{x:40,y:2,z:-10,c:0x20ff80,i:2.4,d:14},
    {x:-45,y:1,z:5,c:0x80cc40,i:1.8,d:13},{x:5,y:2,z:50,c:0x40ff60,i:2.1,d:15},
  ];
  cfgs.forEach(c=>{
    const pl = new THREE.PointLight(c.c, c.i, c.d);
    pl.position.set(c.x, c.y, c.z); scene.add(pl);
    const gm = new THREE.Mesh(new THREE.SphereGeometry(.12,6,6), new THREE.MeshBasicMaterial({color:c.c}));
    gm.position.copy(pl.position); scene.add(gm);
...
function buildRuins() {
  ...
  for (let p=0;p<8;p++) {
    const a=(p/8)*Math.PI*2, px2=35+Math.cos(a)*7.5, pz2=35+Math.sin(a)*7.5, pH=3+Math.random()*5;
    const pi=new THREE.Mesh(new THREE.CylinderGeometry(.4,.5,pH,7),sM);
    pi.position.set(px2,pth+1.5+pH*.5,pz2); pi.castShadow=true; scene.add(pi);
    jungleObjects.push({mesh:pi,radius:.55,x:px2,z:pz2});
...
function buildMushrooms() {
  ...
  for (let i=0;i<50;i++) {
    ...
    const stem=new THREE.Mesh(new THREE.CylinderGeometry(.05,.08,sh,6),new THREE.MeshStandardMaterial({color:0xddccaa,roughness:.9}));
    stem.position.y=sh*.5; g.add(stem);
...
function buildFOBstructures() {
  ...
  const sbPos=[{x:-8,z:5,ry:0},{x:8,z:5,ry:0},{x:0,z:-8,ry:Math.PI/2},{x:-8,z:-5,ry:0},{x:8,z:-5,ry:0}];
  sbPos.forEach(p=>{
  ...
  for (let i=0;i<15;i++) {
    const bx=-40+i*5, bz=-40, bth2=getTerrainHeight(bx,bz);
    const bpost=new THREE.Mesh(new THREE.CylinderGeometry(.04,.04,.9,5),bwM);
    bpost.position.set(bx,bth2+.45,bz); scene.add(bpost);
    if (i>0) {
      const wire=new THREE.Mesh(new THREE.CylinderGeometry(.01,.01,5,3),bwM);
      wire.rotation.z=Math.PI/2; wire.position.set(bx-2.5,bth2+.7,bz); scene.add(wire);
    }
  }
}
...
function buildSoldierMesh(type) {
  const g = new THREE.Group();
  const s = type.scale;
  const uC=type.uniformColor, hC=type.helmetColor, vC=type.vestColor;
  ...
  const vM=new THREE.MeshStandardMaterial({color:vC,roughness:.88,metalness:.08});
  ...
  const lensM=new THREE.MeshStandardMaterial({color:0x003344,roughness:.1,emissive:0x001122,emissiveIntensity:.4});
  ...
  for (let p=0;p<3;p++) {
    ...
    pouch.position.set(px2,.82*s,.13*s); g.add(pouch);
  }
  ...
  const wMat=new THREE.MeshStandardMaterial({color:0x2a2a2a,roughness:.4,metalness:.75});
...
function buildWeaponModel() {
  ...
  if (STATE.currentWeapon===3) buildStingerModel();
  else buildRifleModel(WEAPONS[STATE.currentWeapon]);
...
function buildRifleModel(w) {
  ...
  const mag2=new THREE.Mesh(new THREE.BoxGeometry(.038,.14,.07),rMat); mag2.position.set(0,-.11,-.14); weaponGroup.add(mag2);
  if (STATE.currentWeapon===2) {
    ...
    [-.03,.03].forEach(bx2=>{
      ...
      bl.position.set(bx2,-.04,-.6); bl.rotation.x=.4; weaponGroup.add(bl);
    });
    const stk2=new THREE.Mesh(new THREE.BoxGeometry(.055,.07,.2),rMat); stk2.position.set(0,-.025,.1); weaponGroup.add(stk2);
    const butt2=new THREE.Mesh(new THREE.BoxGeometry(.07,.09,.04),rMat); butt2.position.set(0,-.02,.22); weaponGroup.add(butt2);
  } else {
    const stk=new THREE.Mesh(new THREE.BoxGeometry(.055,.08,.22),rMat); stk.position.set(0,-.02,.14); weaponGroup.add(stk);
  ...
  muzzleFlashLight=new THREE.PointLight(0xffaa30,0,4.5); muzzleFlashLight.position.set(0,0,-.78); weaponGroup.add(muzzleFlashLight);
...
function buildStingerModel() {
  ...
  const grip2=new THREE.Mesh(new THREE.BoxGeometry(.065,.12,.06),new THREE.MeshStandardMaterial({color:0x1a1a1a,roughness:.88})); grip2.position.set(0,-.1,-.18); grip2.rotation.x=.2; weaponGroup.add(grip2);
  ...
  muzzleFlashMesh=new THREE.Mesh(new THREE.SphereGeometry(.04,4,3),new THREE.MeshBasicMaterial({color:0xff8800,transparent:true,opacity:0}));
  muzzleFlashMesh.position.set(0,0,-.7); weaponGroup.add(muzzleFlashMesh);
...
function createFighterJet(startX, startZ, altitude) {
  ...
  const cMat=new THREE.MeshStandardMaterial({color:0x112233,roughness:.1,emissive:0x001122,emissiveIntensity:.6});
  const camMat=new THREE.MeshStandardMaterial({color:0x3a4a32,roughness:.85,metalness:.05});
  ...
  const gLight2=new THREE.PointLight(0x00ff00,.8,3); gLight2.position.set(3.8,.1,.0); g.add(gLight2);
...
function spawnJet(wave) {
  const angle=Math.random()*Math.PI*2, dist=100+Math.random()*50;
  ...
  const el=document.getElementById('aircraftWarning');
  el.classList.remove('hidden');
  setTimeout(()=>el.classList.add('hidden'),4000);
  showNotif('⚠ HOSTILE AIRCRAFT — SWITCH TO STINGER (4)!');
}
...
function updateJets(dt, t) {
  jets.forEach(jet=>{
    ...
    if (jet.phase==='approach') {
      ...
      jet.mesh.position.x+=(cx-jet.mesh.position.x)*dt*.5;
      jet.mesh.position.z+=(cz-jet.mesh.position.z)*dt*.5;
      jet.mesh.position.y+=(targetAlt-jet.mesh.position.y)*dt*.8;
      if (dist<100) jet.phase='strafe';
    }
    if (jet.phase==='strafe') {
      const dir=toPlayer.clone().normalize(); dir.y=-0.15;
      jet.mesh.position.addScaledVector(dir,speed*dt);
      jet.mesh.position.y=Math.max(15,jet.mesh.position.y);
      jet.strafing=true; jet.afterburnerOn=true;
      if (dist<20||jet.mesh.position.y<18) jet.phase='pullout';
    }
    if (jet.phase==='pullout') {
      jet.mesh.position.y+=speed*.8*dt;
      const evadeDir=toPlayer.clone().negate().normalize(); evadeDir.y=0.4;
    ...
    if (jet.phase==='bomb') {
      jet.bombTimer-=dt;
      if (jet.bombTimer<=0){dropBomb(jet.mesh.position.clone());jet.bombTimer=8+Math.random()*6;jet.phase='approach';}
    }
    ...
    jet.mesh.traverse(child=>{
      if (child.name==='afterburner') child.material.opacity=jet.afterburnerOn?.7+Math.random()*.3:0;
      if (child.isPointLight) child.intensity=Math.sin(t*3)>.5?.8:.2;
    });
    jet.shootTimer-=dt;
    if (jet.shootTimer<=0&&jet.strafing&&dist<80) {
      ...
      if (b){b.active=true;b.mesh.visible=true;b.mesh.position.copy(muzzPos);b.vel.copy(shotDir).multiplyScalar(60);b.life=3;b.damage=18;b.isEnemy=true;b.mesh.material.color.set(0xffaa00);}
    }
    if (jet.phase==='strafe'&&dist<120&&Math.random()<.002) launchMissile(jet.mesh.position.clone(),camera.position.clone());
    const b2=MAP_SIZE*.7;
    jet.mesh.position.x=Math.max(-b2,Math.min(b2,jet.mesh.position.x));
    jet.mesh.position.z=Math.max(-b2,Math.min(b2,jet.mesh.position.z));
    if (jet.hitTimer>0) jet.mesh.traverse(c=>{if(c.isMesh&&c.material.emissive)c.material.emissive.set(0xff2200);});
    else jet.mesh.traverse(c=>{if(c.isMesh&&c.material.emissive)c.material.emissive.set(0x000000);});
  });
  updateBombs(dt); updateMissiles(dt); updateStinger(dt);
}
...
function updateBombs(dt) {
  for (let i=bombObjects.length-1;i>=0;i--) {
    ...
    b.vel.y-=12*dt; b.mesh.rotation.z+=2*dt;
...
function launchMissile(from,to) {
  ...
  const nose2=new THREE.Mesh(new THREE.ConeGeometry(.06,.25,5),mat); nose2.position.y=.35; mesh.add(nose2);
  mesh.position.copy(from);
  const dir=new THREE.Vector3().subVectors(to,from).normalize();
  const pl=new THREE.PointLight(0xff6600,3,5); pl.position.set(0,-.3,0); mesh.add(pl);
  scene.add(mesh);
  missileObjects.push({mesh,vel:dir.clone().multiplyScalar(20),life:5,homing:true});
  showNotif('⚠ MISSILE INCOMING — MOVE!');
}
...
function updateMissiles(dt) {
  for (let i=missileObjects.length-1;i>=0;i--) {
    ...
    if (dp<2){applyPlayerDamage(45);spawnExplosionEffect(m.mesh.position.clone(),8);scene.remove(m.mesh);missileObjects.splice(i,1);continue;}
    const th=getTerrainHeight(m.mesh.position.x,m.mesh.position.z);
    if (m.mesh.position.y<th+.3||m.life<=0){spawnExplosionEffect(m.mesh.position.clone(),8);scene.remove(m.mesh);missileObjects.splice(i,1);}
  }
}

/* ================================================================
   SECTION 9: STINGER LOCK-ON
================================================================ */
...
function updateStinger(dt) {
  if (STATE.currentWeapon!==3) {
    ...
    const ss=document.getElementById('stingerStatus'); ss.className='stinger-status'; ss.textContent='';
    STATE.stingerLocked=false; STATE.stingerTarget=null; return;
  }
...
function fireStinger() {
  ...
  const pLight2=new THREE.PointLight(0xff6600,5,8); pLight2.position.copy(camera.position);
  scene.add(rMesh); scene.add(pLight2);
  let prog=0;
  ...
  const rInterval=setInterval(()=>{
    ...
    if (!target.alive||prog>=1){
      ...
      scene.remove(rMesh); scene.remove(pLight2);
      if (target.alive){
        target.health-=250; target.hitTimer=.3;
        showHitMarker(target.health<=0);
        if (target.health<=0) destroyJet(target);
        else showNotif('HIT! JET DAMAGED!');
      }
      spawnExplosionEffect(rMesh.position.clone(),15); return;
    }
    const pos=new THREE.Vector3().lerpVectors(startPos,target.mesh.position.clone().add(new THREE.Vector3(0,1,0)),prog);
    pos.x+=(Math.random()-.5)*.5; pos.y+=(Math.random()-.5)*.5;
    rMesh.position.copy(pos); pLight2.position.copy(pos);
...
function destroyJet(jet) {
  jet.alive=false;
  STATE.score+=jet.score*STATE.wave; STATE.jetKills++;
  showNotif(`★ JET DESTROYED! +${jet.score*STATE.wave} pts`);
  addKillFeedEntry('ENEMY FIGHTER JET',false,true);
  updateScoreUI();
...
function spawnWave(w) {
  ...
  showWaveAnnounce(w);
  showNotif(`WAVE ${w} — ${count} TROOPS${w>=3?' + AIR SUPPORT':''}`);
}

function selectEnemyType(wave,index,total) {
  if (wave>=5&&index===0) return 4;
  if (wave>=3&&Math.random()<.15) return 3;
  if (wave>=2&&Math.random()<.2) return 2;
  if (Math.random()<.3) return 5;
  if (Math.random()<.4) return 1;
  return 0;
}
...
function spawnEnemy(typeIndex) {
  ...
  g.position.set(ex,ey,ez); scene.add(g);
  enemies.push({
...
function throwGrenade() {
  if (STATE.grenades<=0){showNotif('NO GRENADES!');return;}
  STATE.grenades--; updateGrenadeUI();
  const dir=new THREE.Vector3(); camera.getWorldDirection(dir);
  const pos=camera.position.clone().add(dir.clone().multiplyScalar(.8)); pos.y+=.2;
  const mesh=new THREE.Mesh(new THREE.SphereGeometry(.08,7,6),new THREE.MeshStandardMaterial({color:0x3a4a2a,roughness:.7,metalness:.65}));
...
function updateGrenades(dt) {
  for (let i=grenadeObjects.length-1;i>=0;i--) {
    ...
    g.life-=dt;
    if (g.life<=0||g.bounces>5){explodeGrenade(g);scene.remove(g.mesh);grenadeObjects.splice(i,1);}
  }
}
...
function updateEnemies(dt, t) {
  enemies.forEach(e=>{
    ...
    if (e.coverTimer<0&&e.state==='chase'&&Math.random()<.01&&dist<30){
      e.inCover=true;e.crouching=true;e.coverTimer=3+Math.random()*4;e.state='shoot';
    }
    ...
    if (e.throwGrenadeTimer<=0&&dist<25&&dist>8) {
      ...
      const gm=new THREE.Mesh(new THREE.SphereGeometry(.08,7,6),new THREE.MeshStandardMaterial({color:0x3a4a2a,roughness:.7,metalness:.65}));
    ...
    if (e.hitTimer>0) {
      e.hitTimer-=dt;
      e.mesh.traverse(c=>{if(c.isMesh&&c.material.emissive){c.material.emissive.set(0xff2200);c.material.emissiveIntensity=.6;}});
    } else {
...
function checkWaveClear() {
  ...
  const jAlive=jets.filter(x=>x.alive);
  if (alive.length===0&&jAlive.length===0&&STATE.phase==='playing') {
    setTimeout(()=>{
      STATE.wave++;
      enemies.length=0; jets.length=0;
      bombObjects.forEach(b=>scene.remove(b.mesh)); bombObjects.length=0;
      missileObjects.forEach(m=>scene.remove(m.mesh)); missileObjects.length=0;
...
function tryShoot() {
  ...
  setTimeout(()=>{if(weaponGroup){weaponGroup.position.z-=.022;weaponGroup.rotation.x+=.038;}},80);
...
function doMuzzleFlash() {
  ...
  setTimeout(()=>{if(muzzleFlashLight)muzzleFlashLight.intensity=0;if(muzzleFlashMesh)muzzleFlashMesh.material.opacity=0;},50);
}
...
function startReload() {
  ...
  if (weaponGroup) weaponGroup.rotation.x=.5;
...
function updateADS(dt) {
  ...
  if (weaponGroup&&STATE.currentWeapon!==3) {
...
function switchWeapon(idx) {
  ...
  ['ws1','ws2','ws3','ws4'].forEach((id,i)=>{
    ...
    if (el) el.className='wsw-item'+(i===idx?' active':'');
  });
  updateAmmoUI();
  STATE.isADS=false; exitADS();
  if (idx===3) {
    document.getElementById('stingerReticle').className='stinger-reticle';
    document.getElementById('stingerStatus').className='stinger-status visible';
    document.getElementById('stingerStatus').textContent='SEEKING TARGET...';
  }
}
...
function updatePlayer(dt) {
  ...
  if (weaponGroup&&!STATE.isADS&&STATE.currentWeapon!==3){
    weaponGroup.position.y=-.17+bobY; weaponGroup.position.x=.19+bobX;
  }
  if (weaponGroup) weaponGroup.rotation.z=-pitch*.08;
...
function drawMinimap() {
  ...
  jets.forEach(jet=>{
    ...
    mmCtx.fillStyle='#00aaff'; mmCtx.beginPath();
    mmCtx.moveTo(mx,mz-5); mmCtx.lineTo(mx-4,mz+3); mmCtx.lineTo(mx+4,mz+3); mmCtx.closePath(); mmCtx.fill();
  });
...
function updateEnvironment(dt) {
  ...
  pointLights.forEach(pl=>{
    pl.time+=dt*(1.5+Math.random()*.5);
    pl.light.intensity=pl.base*(.7+.4*Math.sin(pl.time*3.1)*Math.cos(pl.time*1.7));
  });
  if (sunLight) {
    sunLight.position.x=Math.cos(envTime*.01)*50; sunLight.position.y=40+Math.sin(envTime*.008)*20;
    const df=Math.max(.3,Math.sin(envTime*.008)+.6);
    sunLight.intensity=.8*df; ambientLight.intensity=1+df*.8;
  }
}
...
function updateTimer() {
  ...
  document.getElementById('hudTimer').textContent=ts;
  document.getElementById('sbTime').textContent=ts;
}

function showNotif(msg) {
  const c=document.getElementById('notifCenter'), d=document.createElement('div');
  d.className='notif-item'; d.textContent=msg; c.appendChild(d);
...
function addKillFeedEntry(name,isCommander,isJet) {
  ...
  e.textContent=isJet?`✈ JET DOWNED`:isCommander?`★ COMMANDER KIA — ${name}`:`✖ ${name} KIA`;
  feed.appendChild(e); setTimeout(()=>{if(e.parentNode)e.remove();},4200);
}
...
function showWaveAnnounce(w) {
  ...
  sub.textContent=w===1?'GROUND TROOPS INCOMING':w>=3?'⚠ GROUND + AIR ASSAULT':'REINFORCEMENTS INCOMING';
...
function showGameover() {
  ...
  const m=Math.floor(STATE.elapsedTime/60),s=Math.floor(STATE.elapsedTime%60);
  document.getElementById('goTime').textContent=String(m).padStart(2,'0')+':'+String(s).padStart(2,'0');
...
function setupMenu() {
  ...
  document.getElementById('btnCredits').addEventListener('click',()=>showNotif('AMAZON STRIKE — Three.js r128 | 6 Enemy Types | Air Combat | Stinger System'));
}
...
function animate() {
  ...
  if (STATE.phase==='playing') {
    ...
    updateParticles(dt);
    updateGrenades(dt);
    updateJets(dt,clock.getElapsedTime());
...
const loadSteps=[
  ...
  {pct:30, msg:'Planting 280 Tropical Trees...'},
  {pct:38, msg:'Growing Ferns, Vines & Vegetation...'},
  {pct:45, msg:'Building Ancient Ruins & FOB Structures...'},
  {pct:52, msg:'Deploying Sandbag Fortifications...'},
  {pct:58, msg:'Lighting Jungle (8 Bioluminescent Points)...'},
  {pct:65, msg:'Assembling Realistic Soldiers (6 Types)...'},
  {pct:71, msg:'Building Fighter Jet (F-16 Inspired)...'},
  {pct:77, msg:'Loading M4A1, AWM Sniper, SAW, Stinger...'},
  {pct:82, msg:'Programming Enemy AI & Cover System...'},
  {pct:87, msg:'Initializing Stinger Lock-On System...'},
  {pct:92, msg:'Configuring Bomb & Missile Physics...'},
...
function tickLoader() {
  if (loadStep>=loadSteps.length){
    setTimeout(()=>{document.getElementById('loadingScreen').style.display='none';setupMenu();},600);
    return;
  }
